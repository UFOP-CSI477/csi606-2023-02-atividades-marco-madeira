...
import { useCreateCity, useRemoveCity } from "@/service/City/city.mutations";
import { CreateCityDTO } from "@/types/City/City";
import { useState } from "react";

export default function CityPage() {
  const { mutate: create } = useCreateCity();
//   const { mutate: update } =;
  const { mutate: remove } = useRemoveCity();

  const [createCity, setCreateCity] = useState<CreateCityDTO>({
    name: "",
    state_id: 0,
  });
  ...
  return (
    <div className="w-1/2 self-center justify-center border-s-white border-2 p-4 flex flex-col gap-y-6">
      <div className="gap-y-3 flex flex-col">
        <p className="text-3xl">Adicionar cidade</p>
        <input
          type="text"
      ...
      <div className="gap-y-3 flex flex-col">
        <p className="text-3xl">Remover cidade</p>
        <input
          type="text"
          className="bg-transparent p-2 border-s-white border-2"
          placeholder="Id"
